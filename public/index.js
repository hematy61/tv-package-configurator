...
// ===============================================================
var groupsNames = [{ name: "Adventure",
	class: "adventure",
	price: 4,
	...
	class: "sports-fans",
	price: 15,
	channels: ["Fight Network", "Sportsnet - Sportsnet 4K also available", "FOX Sports Racing", "OLN", "NFL Network", "Sportsnet 590 The FAN", "Sportsnet 360", "Golf Channel", "Sportsnet ONE - Sportsnet One 4K also available", "MLB Network"],
	channelsPrice: ["$1.99", "$25.00", "$2.99", "$6.00", "$4.00", "$1.99", "$10.00", "$4.00", "$14.00", "$2.99"],
	channelsNumber: [170, "171-174", 175, 176, 177, 178, 181, 182, 187, 188],
	ids: ["fight-network", "sportsnet", "fox-sports-racing", "oln", "nfl-network", "sportsnet-590-the-fan", "sportsnet-360", "golf-channel", "sportsnet-one", "mlb-network"]
}, { name: "Super",
	class: "super",
	price: 4,
	...
	price: 3,
	channels: ["CBC Toronto", "City Toronto", "Global Toronto", "CTV Montreal", "NTV St. John's", "Global Maritimes", "PBS Detroit", "CTV Halifax", "CTV Kitchener", "CTV Moncton", "CTV North Bay", "CTV Ottawa", "CTV Sault Ste. Marie", "CTV Sudbury", "CTV Timmins", "CTV Toronto", "CTV Sydney"],
	channelsPrice: ["$2.99", "$2.99", "$2.99", "$2.99", "$2.99", "$2.99", "$2.99", "$2.99", "$2.99", "$2.99", "$2.99", "$2.99", "$2.99", "$2.99", "$2.99", "$2.99", "$2.99"],
	channelsNumber: [341, 342, 343, 344, 345, 346, 348, 1349, 1350, 1351, 1352, 1353, 1354, 1355, 1356, 1357, 1358],
	ids: ["cbc-toronto", "city-toronto", "global-toronto", "ctv-montreal", "ntv-st-Johns", "global-maritimes", "pbs-detroit", "ctv-halifax", "ctv-kitchener", "ctv-moncton", "ctv-north-bay", "ctv-ottawa", "ctv-sault-ste-marie", "ctv-sudbury", "ctv-timmins", "ctv-toronto", "ctv-sydney"]
}, { name: "Time Shift West",
	class: "time-shift-west",
	price: 3,
	channels: ["City Saskatchewan", "CBC Vancouver", "CTV BC", "Global BC", "City Edmonton", "City Vancouver", "ABC Seattle", "CBS Seattle", "FOX Seattle", "NBC Seattle", "PBS Seattle", "CTV Calgary", "CTV Regina"],
	channelsPrice: ["1.99$", "$2.99", "$2.99", "$2.99", "$2.99", "$2.99", "$2.99", "$2.99", "$2.99", "$2.99", "$2.99", "$2.99", "$2.99"],
	channelsNumber: [330, 331, 332, 333, 334, 335, 336, 337, 338, 339, 340, 1359, 1360],
	ids: ["city-saskatchewan", "cbc-vancouver", "ctv-bc", "global-bc", "city-edmonton", "city-vancouver", "abc-seattle", "cbs-seattle", "fox-seattle", "nbc-seattle", "pbs-seattle", "ctv-calgary", "ctv-regina"]
}, { name: "Variété",
	class: "variete",
	price: 3,
	channels: ["Historia", "Prise 2", "CASA", "euronews (French)", "Investigation", "MusiquePlus", "Max"],
	channelsPrice: ["$1.99", "$1.99", "$1.99", "$1.99", "$4.00", "$4.00", "$4.00"],
	channelsNumber: [431, 432, 433, 434, 435, 436, 437],
	ids: ["historia", "prise-2", "casa", "euronews-French", "investigation", "musique-plus", "max"]
}];
...
function elementAndAttrMaker(type, attr, val, text) {
	...
	attr.map(function (currentValue, index) {
		var attr = document.createAttribute(currentValue);
...
function groupCreator(group) {
	var channelGroups = document.getElementsByClassName("channel-groups")[0];
	var div = document.createElement("div");
	var groupClass = document.createAttribute("class");
...
function legendMaker(legendId) {
	...
	var inputAttributesObjesct = {
		attr: ["type", "id", "name", "value"],
		val: ["checkbox", legendId.class, "channelGroup", legendId.class]
	};
	var input = elementAndAttrMaker("input", inputAttributesObjesct.attr, inputAttributesObjesct.val);
	legend.appendChild(input);

	// Creating legend's <label> element
	var label = elementAndAttrMaker("label", ["for"], [legendId.class], legendId.name);
	legend.appendChild(label);

	// Creating legend's <p> element
	var p = elementAndAttrMaker("p", [], [], "$" + legendId.price);
...
function channelMaker(group, fieldset) {
	group.channels.map(function (channel, index) {

		var divChannel = elementAndAttrMaker("div", ["class"], ["channel"]);
		var divChannelNameRow = elementAndAttrMaker("div", ['class'], ['channel-name-row']);
		var input = elementAndAttrMaker("input", ['type', 'id', 'name', 'val'], ['checkbox', group.ids[index], "channel", group.ids[index]]);
		divChannelNameRow.appendChild(input);

		var label = elementAndAttrMaker("label", ["class", "for"], ["channel-number", group.ids[index]], group.channelsNumber[index]);
		...
		var p = elementAndAttrMaker("p", [], [], group.channelsPrice[index]);
...
var legend = document.querySelectorAll("legend input");
legend.forEach(function (eachLegend) {
	// handeling group selecting
	eachLegend.addEventListener("click", function () {
		var channelsGroup = document.querySelectorAll("div." + eachLegend.id + " input");
		channelsGroup.forEach(function (channel, index) {
	...
	var channels = document.querySelectorAll("div." + eachLegend.id + " div.channel-name-row input");
